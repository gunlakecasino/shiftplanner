import { MoreHorizontal } from "lucide-react";
import { ZONE_COLORS, ZONE_STATUS } from "../tokens";
import type { BackstageCardProps } from "../types";

export function BackstageCard({ zone, label, coveredBy, name, notes, zoneBanner, onClick }: BackstageCardProps) {
  const colors = ZONE_COLORS[zone] || ZONE_COLORS[1];
  const accentColor = colors.label;

  return (
    <div
      onClick={onClick}
      className="rounded-xl border border-gray-200 min-h-[172px] flex overflow-hidden shadow-sm cursor-pointer hover:shadow-md transition-shadow bg-white"
    >
      <div className="w-[5px] shrink-0 rounded-l-xl" style={{ backgroundColor: accentColor }} />
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        <div className="flex flex-col flex-1 p-3 min-w-0">
          <div className="flex items-center justify-between mb-2.5">
            <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-md" style={{ backgroundColor: `${accentColor}18`, color: accentColor }}>
              {label ?? `ZONE ${zone}`}
            </span>
            <div className="flex items-center gap-1.5">
              <div className={`w-2 h-2 rounded-full ${ZONE_STATUS[zone] ?? "bg-green-400"}`} />
              <MoreHorizontal size={12} className="text-gray-400" />
            </div>
          </div>
          {coveredBy && (
            <div className="text-[9px] font-bold uppercase tracking-widest mb-1" style={{ color: accentColor, opacity: 0.6 }}>{coveredBy}</div>
          )}
          <div className="text-[17px] font-bold text-gray-900 leading-tight truncate mb-2">{name}</div>
          {notes && notes.length > 0 && (
            <div className="mt-auto flex flex-col gap-1">
              {notes.map((n, i) => (
                <div key={i} className="flex items-start gap-1.5">
                  <div className="w-1 h-1 rounded-full mt-[5px] shrink-0" style={{ backgroundColor: accentColor, opacity: 0.5 }} />
                  <span className="text-[10px] text-gray-500 leading-snug truncate">{n}</span>
                </div>
              ))}
            </div>
          )}
        </div>
        {zoneBanner && (
          <div className="px-3 py-1 flex items-center justify-center" style={{ backgroundColor: accentColor }}>
            <span className="text-[8px] font-bold text-white tracking-widest uppercase">{zoneBanner}</span>
          </div>
        )}
      </div>
    </div>
  );
}
