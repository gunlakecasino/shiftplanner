...
INSTANT PRERENDER: When PRERENDER block is present, treat it as the default verdict. You MAY change fitVerdict/fitSummary only if deterministic facts clearly support a different judgment — then set verdictOverrideReason (one short sentence). Questionable only when a better-suited placement or bilateral swap exists that improves rotation health net-wide; do NOT suggest rotation with a TM who is strong_fit or acceptable on their slot, or where the trade would make them questionable/poor_fit. needs_swap only for bilateral lanes where both sides benefit. Repeat exposure (2×+ in 30, or in last-5 trail) alone is acceptable if no better gap — use strong_fit for 0–1× spread and not in last-5. Unassigned slots: fitSummary must name one best pick; swapRecommendations MUST be [].
