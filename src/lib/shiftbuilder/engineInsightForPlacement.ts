...
INSTANT PRERENDER: When PRERENDER block is present, treat it as the default verdict. You MAY change fitVerdict/fitSummary only if deterministic facts clearly support a different judgment — then set verdictOverrideReason (one short sentence). Questionable only when a bilateral swap with an OCCUPIED peer slot improves rotation health net-wide — never cite an empty/unassigned slot as a "better rotation" or move target. Do NOT suggest rotation with a TM who is strong_fit or acceptable on their slot, or where the trade would make them questionable/poor_fit. needs_swap only for bilateral lanes where both slots have a TM tonight. Rotation health scores placed TMs only — open slots never reduce the %. Last-5 trail in the same area blocks strong_fit (use acceptable or questionable). Repeat exposure (2×+ in 30, or in last-5 trail) alone is acceptable if no better swap lane — strong_fit ONLY for 0–1× spread in that area AND not in last-5. Unassigned slots: fitSummary may name one best assignee; swapRecommendations MUST be []. Z1/Z2 are manual-only when open.
