...
import { getFullyResolvedEngineConfigServer } from "./engineConfig.server";
import {
...
async function loadWeekPlacementEntries(
  ...
  const { data: assignRows, error } = await admin()
 ...
 */
export async function batchRunEngineForWeekServer(
  weekId: string,
    ...
    try {
      ...
      if (nightResult.status === "ok" && nightResult.assigned > 0) {
        const { data: freshRows } = await admin()
          .from("zone_assignments")
          .select("slot_key, slot_type, rr_side, tm_id")
          .eq("night_id", nightId)
          .not("tm_id", "is", null);
        ...
        for (const a of freshRows ?? []) {
          ...
          try {
            ...
            proposed[uiKey] = String(row.tm_id);
          } catch {
            /* skip */
          }
        }
 ...
 */
export async function batchRunEngineForNightServer(
  nightId: string,
  options: BatchRunOptions = {}
): Promise<BatchNightResult> {
  const { skipFilledNights = false, requireSchedule = false, filterBySchedule = false } = options;

  const { data: nightRow, error: nightErr } = await admin()
    .from("nights")
    .select("id, night_date, day_name")
    .eq("id", nightId)
    .maybeSingle();
  ...
  const { data: weekNightRows } = await admin()
    .from("nights")
...
async function runEngineForSingleNight(params: {
  ...
  dayName: string;
  grave: GraveTm[];
  engineConfig: Awaited<ReturnType<typeof getFullyResolvedEngineConfigServer>>;
  ...
  // Load night-specific data via admin (session APIs may not expose these from server process).
  ...
  const [scheduledIds, existingAssignments] = await Promise.all([
    ...
    (async () => {
      const { data, error } = await admin()
        .from("zone_assignments")
        .select("slot_key, slot_type, rr_side, tm_id, is_locked, tm_profiles(display_name)")
        .eq("night_id", nightId);
      if (error) throw new Error(`load assignments: ${error.message}`);
      return (data ?? []).map((row: Record<string, unknown>) => ({
        slotKey: String(row.slot_key ?? ""),
        slotType: String(row.slot_type ?? "zone"),
        rrSide: (row.rr_side as string | null) ?? null,
        tmId: row.tm_id ? String(row.tm_id) : null,
        tmName:
          (row.tm_profiles as { display_name?: string } | null)?.display_name ??
          (row.tm_id ? String(row.tm_id) : null),
  ...
  // Count preserved / unfilled; collect non-preserved proposals for session-safe write
  let preserved = 0;
  let assigned = 0;
  let unfilled = 0;
  const slotsToWrite: Array<{
    slotKey: string;
    slotType: string;
    rrSide: string | null;
    tmId: string | null;
  ...
  if (slotsToWrite.length > 0) {
    try {
      const { batchApplyDraftAssignmentsServer } = await import(
        "@/lib/shiftbuilder/opsMutations.server"
      );
      await batchApplyDraftAssignmentsServer(nightId, slotsToWrite, nightDate);
      assigned = slotsToWrite.length;
    } catch (writeErr) {
      const msg = writeErr instanceof Error ? writeErr.message : String(writeErr);
      notes.push(`Write rejected: ${msg}`);
      return {
        nightId,
        nightDate,
        dayName,
 ...
 */
export async function listNightsForWeekServer(
  weekId: string
 ...
 */
export async function listWeeksWithNightsServer(): Promise<
  Array<{ weekId: string; weekEnding: string; weekLabel: string; nightCount: number }>
> {
  const { data: weeks, error: weekErr } = await admin()
    .from("weeks")
  ...
  const { data: nightCounts } = await admin()
    .from("nights")
    .select("week_id")
    .in("week_id", weekIds);
